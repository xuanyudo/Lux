...
class JSON_S:
    ...
    def update_req(self):
        ...
        d_string = d_string[:-1]
        c_string = '{"cmd": '+'"'+str(Data.command)+'",'+'"uuid": '+Data.uuid+','+'"data":{'+d_string+'}}'
        return c_string
    def status_req(self):
...
class connection:
    ...
    def sendToServer(self):             #The function that encode a JSON string to the server base on command
        json_s = JSON_S()
        rawMessage = json_s.switcher(Data.command)
...
def login_page():
    if request.method == 'POST':
            ...
            if(identity == Data.login_info["id"] and password == Data.login_info["pass"]):
                # print(jsonify({"login": "success"}))
                return jsonify({"login": "success"})
            else:
                # print(jsonify({"login": "failed"}))
